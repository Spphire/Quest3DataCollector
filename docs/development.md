...
- Unity sends raw Unity world telemetry (`unity_world_lh_y_up_z_forward`). PC code keeps those raw samples for traceability and derives a canonical display/robot frame (`pc_world_rh_z_up_x_forward`) with `pc = [unity.z, -unity.x, unity.y]`. This frame is right-handed, has +Z aligned with gravity/up, and matches the Flexiv/robot convention. Legacy snapshots tagged `pc_world_rh_y_up_z_back` are converted when loaded. Apply frame conversion only at PC boundaries: live visualization, replay payloads, robot alignment, and controller teleoperation.
...
- Right-controller motion uses Flexiv RDK v1.7 non-real-time Cartesian motion-force mode with all force-control axes disabled. It reads the controller in the canonical PC frame and maps through `T_base_world`; A-button robot teleoperation requires this Quest-robot alignment to be available. When running without a solved alignment for diagnostics, the fallback mapping is identity in the canonical PC/Flexiv Z-up frame, so controller +Z commands robot +Z.
- B-button calibration free-drag first tries Flexiv's native `FloatingCartesian()` primitive in `NRT_PRIMITIVE_EXECUTION`. If the primitive is unavailable or not licensed, it falls back to `NRT_CARTESIAN_MOTION_FORCE` with low Cartesian impedance and a persistent 100 Hz control loop that continuously sends the current TCP pose. The fallback mirrors the working iPhone calibration receiver pattern and avoids relying on `PLAN-FreeDriveManual` or `NRT_JOINT_IMPEDANCE` zero stiffness, which can report enabled without producing physical drag on this setup.
