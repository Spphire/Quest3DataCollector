...
def main() -> int:
    ...
    args.image_y_axis = resolve_requested_image_y_axis(records, raw_root, str(args.image_y_axis))

    detection_summary = []
    emit_progress("start", 0.0, records=records, outputRoot=str(output_root), imageYAxis=args.image_y_axis)
...
def resolve_requested_image_y_axis(records: list[str], raw_root: Path, requested_axis: str) -> str:
    if requested_axis != "auto":
        return requested_axis
    for record in records:
        metadata_path = raw_root / record / "quest_camera_metadata.json"
        try:
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        except Exception:
            return requested_axis
        video_frame_convention = str(metadata.get("videoFrameConvention") or "").strip().lower()
        pc_receiver_orientation = str(metadata.get("pcReceiverVideoOrientation") or "").strip().lower()
        if video_frame_convention != "opencv_top_left_y_down" and pc_receiver_orientation != "canonical_top_left_y_down":
            return requested_axis
    return "down" if records else requested_axis
...
def scan_video(record_dir: Path, side: str, metadata: dict[str, Any], out_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    corner_indices = checkerboard_corner_indices(args.pattern_cols, args.pattern_rows)
        ...
        if meta is not None and (candidate_frame_indices is None or frame_index in candidate_frame_indices):
            ...
            if found and corners is not None:
                ...
                appearance_anchor_luminance_by_corner.append(
                    np.asarray([float(luminance.get(str(idx), np.nan)) for idx in corner_indices], dtype=np.float64)
                )
    ...
    return {
        ...
        "appearance_anchor": {
            ...
            "corner_indices": [int(v) for v in corner_indices],
            "min_contrast": float(args.appearance_anchor_min_contrast),
        },
...
def _fit_per_record_fixed_y_axis(records: list[str], raw_root: Path, detections_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "corner_order_policy": (
            "per-frame prefer checker black/white appearance anchor, then lower median reprojection error "
            "to resolve identity vs rot180; discard weak frames; refit"
        ),
